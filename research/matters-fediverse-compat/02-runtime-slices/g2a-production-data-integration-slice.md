...
| `matters-server` | `npm run federation:export -- --enforce-federation-gate` | optional strict mode exists in commit `3497556`; default preflight export remains public-only, while strict mode additionally requires explicit author opt-in and article settings |
