...
| Bounded production pilot `Update` | Cleared | One gateway-origin `Update` for article `1225211` was sent on 2026-05-28 and delivered to the same two accepted pilot followers. A later bounded production `Update` for article `1228008` was sent on 2026-06-05 after the pilot-outbound decision and delivered to g0v.social, gyutte.site, and Threads while production server-triggered outbound stayed disabled. Follow-up queue cleanup resolved two obsolete Threads compatibility payloads without replay, leaving `pending=0`, `retryPending=0`, and `deadLetter=0`. See `production-pilot-update-run-20260528.md` and `production-pilot-update-run-20260605.md`. |
