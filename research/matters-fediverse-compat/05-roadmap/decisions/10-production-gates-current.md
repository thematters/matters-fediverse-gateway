...
| Production audit repeat query | Waiting on environment approval | Read-only workflow run 26269962135 was triggered on 2026-05-22 with `include_decision_report=false` after the v5.23.0 release, but GitHub production environment approval is required before it can run. |
