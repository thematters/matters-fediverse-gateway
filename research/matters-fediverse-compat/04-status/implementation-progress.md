...
- `cd gateway-core && runtime/tools/node-local --test`
  ...
  2026-06-02 Threads Follow and delivery regression passed gateway-side：PR #90 deployed AWS origin commit `8db7477` and changed Follow responses so `Accept.object` / `Reject.object` embed the original Follow activity when available；`npm test` returned 139/139；existing pending Threads Accept queue items replayed as delivered；live followers now include accepted g0v.social, gyutte.site, and Threads rows；live queue snapshot returned `pending=0`、`processing=0`、`deadLetter=0`；latest public Article Create `https://matters.town/ap/activities/1780361487400-create-mashbeanmatters` delivered to Threads sharedInbox. Receiver-visible Threads Article display / reply / like checks remain pending.
