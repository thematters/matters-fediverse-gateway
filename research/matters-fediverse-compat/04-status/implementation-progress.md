...
- `G2-B Product Controls`
  staging validation blocked on permission
  `matters-server` PR #4773 已 merged to develop 並部署到 `matters.icu`；`matters-web` PR #5883 已 merged to develop 並部署到 `matters.icu`；`server.matters.icu` schema 已確認包含 `User.federationSetting`、`Article.federationSetting`、`Article.federationEligibility`、`setViewerFederationSetting`、`setArticleFederationSetting`、`UserFeatureFlagType.fediverseBeta`。read-only GraphQL gate 驗證顯示 public article `23520` 因 author 尚未 opt-in 回 `author_not_opted_in`，paywalled article `23522` 回 `article_not_public`。Lambda public-only workflow run `25700094845` 與 strict-gate row-level workflow run `25700094876` 均成功；strict-gate payload 下 public article `23520` eligible，paywalled article `23522` 仍 blocked。`mashbean@matters.town` 是預定 staging pilot/admin test account，但目前尚未取得 staging admin / `fediverseBeta` 權限，因此 UI opt-in 驗證暫停在權限 gate。
