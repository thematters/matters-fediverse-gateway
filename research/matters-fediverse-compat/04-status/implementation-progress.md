...
- `cd gateway-core && runtime/tools/node-local --test`
  ...
  2026-06-04 Threads receiver-visible readback updated：human UI screenshot confirmed Threads opens the remote profile page for `mashbeanmatters@matters.town`, shows `following`, and displays remote posts in the feed. Threads search still does not reliably find the account, the remote post does not expose a copyable single-post permalink, and Threads UI shows a beta notice that users can like posts from other servers but cannot reply yet. Gateway admin readback confirmed a Threads-origin Like for the visible Note probe: `https://threads.net/ap/users/17841401579146452/#likes/869695086184604` appears under `/admin/local-notifications?actorHandle=mashbeanmatters&category=like`, and `/admin/local-content` for `https://matters.town/ap/notes/1780526263869-threads-note-visibility-mashbeanmatters` reports `metrics.likes=1` / `actionMatrix.inbound.like=1`.
