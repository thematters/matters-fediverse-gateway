...
- `cd gateway-core && runtime/tools/node-local --test`
  ...
  2026-05-17 canonical profile URL repair deployed：gateway-core PR #54 stopped deriving actor profile URLs from the `/ap` ActivityPub path and deployed AWS origin commit `201993b`; Cloudflare Worker PR #55 then fixed pilot WebFinger profile-page aliases and deployed Worker version `675d2601-7960-4a52-bbb2-7d14dc642480`. Live probes now show WebFinger aliases `https://matters.town/@mashbeanmatters` and `https://matters.town/ap/users/mashbeanmatters`; the actor document `url` also returns `https://matters.town/@mashbeanmatters`; `check:follow-readiness -- --base-url https://matters.town --handle mashbeanmatters` returned `ok: true`. At that point Threads could discover the canonical profile but follow did not complete; this was superseded by the 2026-06-02 embedded-Follow Accept fix. PR #57 then made outbound Accept activities dereferenceable through `/ap/activities/*`; AWS origin and Worker were deployed, and the staging actor key id was moved from the old Worker demo `#main-key` to `#gateway-core-20260517`. After gyutte.site remote-user refresh and cancel/re-follow, Misskey `users/relation` returned `isFollowing: true`.
  ...
  2026-06-02 Threads Follow and delivery regression passed gateway-side：PR #90 deployed AWS origin commit `8db7477` and changed Follow responses so `Accept.object` / `Reject.object` embed the original Follow activity when available；`npm test` returned 139/139；existing pending Threads Accept queue items replayed as delivered；live followers now include accepted g0v.social, gyutte.site, and Threads rows；live queue snapshot returned `pending=0`、`processing=0`、`deadLetter=0`；latest public Article Create `https://matters.town/ap/activities/1780361487400-create-mashbeanmatters` delivered to Threads sharedInbox. Receiver-visible Threads post display / reply / like checks remain pending; the 2026-06-03 Note probe delivered successfully but was not observed immediately in Threads UI.
  2026-06-03 Threads Article object-id compatibility narrowed：PR #98 deployed Worker proxying for canonical `/ap/articles/*` object reads；PR #100 deployed gateway-core canonical Article object ids to AWS origin commit `fe3d155`；PR #102 then removed public `atomUri` injection and deployed AWS origin commit `3f2acc8` after `npm test` passed 142/142. Live `/ap/healthz` returned `ok=true` / `followReadiness=ready`, and the successful full canonical Article object remains publicly dereferenceable as ActivityPub JSON with canonical `id`, Matters short `url`, `published`, `updated`, `mediaType`, and no `atomUri`. The earlier missing-fields/atomUri variant remains a bad retryPending test item; do not treat it as a production blocker.
  2026-06-03 post-cleanup live Article probe passed gateway-side：SSM command `e647462a-bd45-4898-b54a-b2bccd4b7e93` sent `https://matters.town/ap/activities/1780529129484-create-mashbeanmatters` to g0v.social、gyutte.site、Threads；all three delivery rows reported `delivered`。The public object `https://matters.town/ap/articles/1228008-threads-article-atomuri-cleanup-20260603T232529Z` returns ActivityPub JSON with canonical `id`, short Matters `url`, `published`, `updated`, `mediaType`, and no `atomUri`。The old missing-fields/atomUri Threads test item moved to dead-letter after its final HTTP 500 retry；live queue is now `pending=0`、`retryPending=0`、`delivered=48`、`deadLetter=1`。Receiver-visible Threads UI readback and Threads-origin reply/like return remain pending.
  2026-06-04 Threads receiver-visible readback updated：human UI screenshots confirmed Threads opens the remote profile page for `mashbeanmatters@matters.town`, shows `following`, displays remote posts in the feed, and exact-handle search for `@mashbeanmatters@matters.town` returns the fediverse account row. Gateway admin readback confirmed a Threads-origin Like for the visible Note probe: `https://threads.net/ap/users/17841401579146452/#likes/869695086184604` appears under `/admin/local-notifications?actorHandle=mashbeanmatters&category=like`, and `/admin/local-content` for `https://matters.town/ap/notes/1780526263869-threads-note-visibility-mashbeanmatters` reports `metrics.likes=1` / `actionMatrix.inbound.like=1`. Follow-up logged-in Safari readback confirmed the latest companion Note is visible, `查看原始貼文` points to `https://matters.town/a/3tmz0u0a42qx`, Share is blocked by `You can't share posts from other servers yet.`, and Reply is blocked by `You can't reply to posts from other servers yet.` Treat those permalink / share / reply gaps as Threads receiver-side beta limitations rather than gateway blockers.
  2026-06-04 Threads Note companion adapter implemented：gateway-core now supports `compatibility.noteCompanion`, disabled by default, requiring actor and receiver-domain allowlists. `Create(Article)` remains the primary fanout, while configured pilot cases can emit a short companion `Create(Note)` only to matched receivers such as `threads.net`; the response includes a `noteCompanion` delivery block. `npm test` passed 144/144. Live enablement remains a separate operator action.
