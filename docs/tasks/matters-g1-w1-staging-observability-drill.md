...
next_step: Cloudflare Tunnel transport smoke 與 staging-style observability drill 已通並封存；下一步需決定 staging 長駐方式與 admin/hooks access policy。
blockers: 需要真人決定是否把 cloudflared/gateway/webhook/proxy 安裝為此 Mac 的長駐服務，或移到小型 VM / container；也需決定 staging-admin 與 staging-hooks 的 Cloudflare Access policy
---
