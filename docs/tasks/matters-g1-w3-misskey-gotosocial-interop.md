...
next_step: Misskey probe 已可本地驗證；下一步需要 mashbean 在 Safari / Misskey Web 建立或提供 `gyutte.site` access token，再以公開 staging gateway URL 實跑並封存 run report。GoToSocial probe 仍待帳號與 token。
blockers: 需要 mashbean 提供 gyutte.site access token、可被 gyutte.site 存取的 gateway public URL，以及 GoToSocial 測試帳號與 access token；token 需遮罩，其餘公開測試證據可保留
---
