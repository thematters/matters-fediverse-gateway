...
next_step: Misskey public resolve / follow / relation probe 已完成；使用者決定 GoToSocial 先不用，直接轉入 W4a 長文 Article 顯示差異整理。
blockers: GoToSocial 依目前決策暫不執行；使用者決定 full-scope Misskey staging token 先保留；Zero Trust 延後處理，暫用本地 admin lockout
---
...
- 2026-05-02 user confirmed Misskey token should be kept for now and Misskey is sufficient for this pass; GoToSocial public run is skipped until a later decision.
