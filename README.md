...
4. Production preparation may proceed for the `mashbean` pilot author in record-only / observation mode, but full outbound delivery remains an implementation task rather than an environment toggle. Mastodon canonical follow proof, Misskey canonical follow convergence, canonical pilot Article visibility, Misskey interaction return, Threads gateway-side Follow/Accept delivery, Threads profile/feed visibility, Threads exact-handle account search, Threads Like return for both the visible Note probe and latest companion Note, and AWS origin offline backup/restore proof have passed. The Threads `Note` companion adapter is enabled only for `mashbeanmatters` and `threads.net`; the 2026-06-04 bounded proof delivered the main `Article` and companion `Note` after PR #108 changed the companion payload to plain text. The expanded Threads discovery regression now verifies exact public activity/object URLs with Meta-like user agents and returned `ok=true` for WebFinger, actor, outbox, Article, Note, and companion activity. Logged-in Threads UI readback confirmed the latest companion Note is visible, `查看原始貼文` points back to `https://matters.town/a/3tmz0u0a42qx`, and Threads currently blocks remote Reply and Share with receiver-side beta notices. Production private S3 storage and public `Create`/`Update`/`Delete` delivery after rollout are approved, but remote visible withdrawal must be described as best-effort because gyutte.site accepted Delete delivery without removing the imported note.
   ...
   - Do not claim full production outbound delivery is enabled until `matters-server` implements a reviewed non-`record_only` trigger path. The current source supports only `off` and `record_only`, so Elastic Beanstalk cannot open full outbound by env-var change alone. The narrow pilot sequence is prepared in [`production-pilot-outbound-runbook.md`](research/matters-fediverse-compat/03-ops/production-pilot-outbound-runbook.md), and the current gates are separated in [`10-production-gates-current.md`](research/matters-fediverse-compat/05-roadmap/decisions/10-production-gates-current.md).
