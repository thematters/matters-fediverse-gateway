...
4. Keep G2-A/G2-B non-production until production credentials, migration timing, legal/privacy review, rollback/takedown checks, and canonical multi-platform follow proof are complete. Mastodon canonical follow proof and AWS origin offline backup/restore proof have passed; Misskey canonical follow and Threads discovery remain open compatibility tracks. Production private S3 storage and public `Create`/`Update`/`Delete` delivery after rollout are approved.
