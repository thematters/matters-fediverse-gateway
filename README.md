...
- G2-B product controls are in review: `matters-server` PR [#4773](https://github.com/thematters/matters-server/pull/4773) adds read-side federation settings plus pilot-scoped author/article mutations, and `matters-web` draft PR [#5883](https://github.com/thematters/matters-web/pull/5883) adds pilot UI controls for account-level opt-in and per-article override
...
2. Continue G2-B from the open product-control PRs: review and merge `matters-server` PR #4773 to `develop`, deploy it to `matters.icu`, then validate and promote `matters-web` PR #5883 out of draft.
