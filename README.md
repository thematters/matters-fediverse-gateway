...
- `matters-server` PR [#4774](https://github.com/thematters/matters-server/pull/4774) is open against `develop` for the export trigger scaffold: `MATTERS_FEDERATION_EXPORT_TRIGGER_MODE=off` by default, optional `record_only` audit rows in `federation_export_event`, no Lambda/S3/IPNS/ActivityPub delivery from the main backend, and non-blocking publish/edit behavior if audit recording fails
...
2. Merge and deploy `matters-server` PR [#4774](https://github.com/thematters/matters-server/pull/4774) to `develop`, then validate `MATTERS_FEDERATION_EXPORT_TRIGGER_MODE=record_only` on `matters.icu` by publishing/editing a pilot public article and confirming `federation_export_event` records the strict decision report without invoking production delivery.
