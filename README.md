...
2. Enable `MATTERS_FEDERATION_EXPORT_TRIGGER_MODE=record_only` on the `matters.icu` develop server environment, redeploy or restart that environment, then validate the trigger by publishing/editing a pilot public article and confirming `federation_export_event` records the strict decision report without invoking production delivery.
