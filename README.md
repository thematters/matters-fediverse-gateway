...
- Current G2-B staging gate: `mashbean@matters.town` is the intended pilot/admin test account, but the account does not yet have staging admin / `fediverseBeta` permission, so real UI opt-in validation is intentionally pending
...
2. Continue G2-B from staging validation: grant `mashbean@matters.town` staging admin / pilot access, add `fediverseBeta`, then validate the account-level and per-article controls on `matters.icu`.
